# TG Export Trade Web — Developer Guide
...
- http://localhost:3000/products/cooking-oil/rbdw-sunflower-oil — product leaf + gallery
- http://localhost:3000/products/rice/jasmine-rice-thai-hom-mali — rice leaf
...
`assets/logo-source.jpeg` is the single source of truth. Every derived asset — `app/favicon.ico`, `app/icon.png`, `app/apple-icon.png`, `app/opengraph-image.png`, `app/twitter-image.png`, and `public/brand/*` — is generated by `npm run brand:assets`. Edit the source file and re-run the script rather than hand-editing outputs.
...
| `SALES_INBOX_EMAIL` | Sales alert recipient (default `sales@tgeptrade.com`) |
...
| `npm run brand:assets` | Rebuild logo/favicon/OG images from `assets/logo-source.jpeg` |
